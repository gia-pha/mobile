...
void main() {
  ...
  test('Get family member', () async {
    // Create a Pact between the consumer and provider
    final pact = PactMockService('mobile', 'genogram');

    var path = PactMatchers.Term(r'/api/members/\d+', '/api/members/5');
    path['pact:generator:type'] = 'ProviderState';
    path['expression'] = '/api/members/\${id}';

    // Create a new interaction
    pact
        .newInteraction(description: 'a request to get family member')
        // Set up the provider state
        .given('family member exist in the system')
        // Configure the request
        .withRequest(
          'GET',
          jsonEncode(path),
    ...
    try {
      ...
      var url = Uri.http(pact.addr, '/api/members/5');
      var response = await http.get(url);
      print('Response status: ${response.statusCode}');
      print('Response body: ${response.body}');
